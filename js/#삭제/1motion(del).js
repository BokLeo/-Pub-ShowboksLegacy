...
// ★ 모든 동작에 들어가는 조작 중 '공통 동작을 위한 조작'이 아닌 단일 플랫폼(pc or mobil)을 위한 조작은 이곳에 기입되어야 한다.
		...
		//------------------------------------------------- 플랫폼 관계(O) & 너비 관계(X) & 스크롤 관계(X) 동작 실행 ※시작
			...
			$(".gnb li:eq(1)").click(function(){$("html,body").stop().animate({scrollTop: 900 });});
			$(".gnb li:eq(2)").click(function(){$("html,body").stop().animate({scrollTop: 3700 });});
			$(".gnb li:eq(3)").click(function(){$("html,body").stop().animate({scrollTop: 5700 });});
		//------------------------------------------------- 플랫폼 관계(O) & 너비 관계(X) & 스크롤 관계(X) 동작 실행 ※끝     
...
// ----------------------------------------------------------------------------리사이즈 감지에 따른 동작 ※시작
	$(window).resize(function(){
		 ...
		 ss2H = headerH + sectionH1
		 ss3H = headerH + sectionH1 + sectionH2
		 ss3H = headerH + sectionH1 + sectionH2 + sectionH3
