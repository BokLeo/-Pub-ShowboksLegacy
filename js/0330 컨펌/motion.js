$(function(){
	...
	var ic = setInterval(" imgChange()",300);
	*/
	//2.	
	/*
	이미지 교체 for반복문 예제
	$(function(){
		var  i 
		for( i = 1; i<=42 ; i++ ){
			 $(".future_visual p:eq(0) img").attr("src", "img/test/test"+ i+".png")
		};
	});
	*/
	//3.
	/*
	각 섹션별 애니메이션 총 시간/현재 재생시간에 맞춰 자동으로 스크롤바 이동.
	$("body").css({ overflow: "hidden" });
	var total = 0;
	var s1 = setInterval( "test()",1000)
	function test(){
			total += 1;
			$("#info p:eq(1)").text("진행중인 애니메이션 시간:"+total+"초");
			if( total == 11){
				total = 0;
				clearInterval(s1);
				$("body,html").animate({ scrollTop:1000 });
				var s2 = setInterval( "test2()",1000)
			}
	}
...
//				$(".future_visual p:eq(0) img").attr("src", "img/test/test"+num+".png");
//         }else{
//				num  =   "finish";   num변수값에 문자열 finish를 대입 하겠다.
//         }
//	};
//
//
...
//-------------------------------------------------------------------------- 체크	
	...
	function mainMotionAll( a , b , c ,   pl, pt, mt ){
		    a.animate({opacity:"1"},1000,function(){
            b.animate({paddingLeft: pl, opacity:"1"},1000,function(){
                b.animate({paddingTop:pt},1500,function(){
                    c.animate({marginTop: mt,opacity:"1"},1500);
                });
            });
        });
	};
	//                                   a                         b                               c                     pl          pt           mt
	mainMotionAll( $(".now") , $(".now .main h3") ,$(".now .main p") , "80px", "50px", "190px");
	mainMotionAll( $(".future") , $(".future .main h3") ,$(".future .main p") , "40px", "95px", "170px");
