...
////	------------------------------------------------------------------------- 체크
	//-------------------------------------------------------------------------- 체크		
		...
		if(bwWidth<=1100){
               ...
               $(".future_visual p img").css({opacity:"0"});
               $(".future_visual p:eq(0) img").attr("src", "img/signature_motion.gif");
...
//				$("body").prepend("<div class='aa'>1</div>");
//			$(".aa").css({position:"absolute",left:0, top:0, background:"rgba(0,0,0,0.5)", 
//							  zIndex:100, width: "30px" , border:"2px solid red"  });
	...
	function allMotion(){
			...
			$(".history_title").click(function(){
				$(this).next().stop().slideDown("fast", function(){
					$(".history_title").next().not(this).stop().slideUp("fast");
				});
...
//									var aaaa =  ss2H + parseInt($(".past_wrap").css("height")) - parseInt($(".career").css("height")) + parseInt( $(".history_info").css("height")) + 280; //    - 1000;
//									$("body").prepend("<div class='aa'>1</div>");
//									$(".aa").css({position:"absolute",left:0, top:0, background:"rgba(0,0,0,0.5)", 
//													  zIndex:100, width: "30px" , height: aaaa, border:"2px solid red"  });
//	
...
// ★ 모든 동작에 들어가는 조작 중 '공통 동작을 위한 조작'이 아닌 단일 플랫폼(pc or mobil)을 위한 조작은 이곳에 기입되어야 한다.
		//------------------------------------------------------ PC 원위치 ※시작 <<<< ★정상으로 돌린 PC조작을★ 붙여넣기
			...
			$(".gnb li").mouseleave(function(){$("li").not(this).stop().css({marginTop:"0px"});});
				...
				$(".skill ul li p").css({opacity:"1", paddingTop:"0"});
		...
		//------------------------------------------------ Moblie 동작을 위한 함수 목록 ※시작 << pcMotion()속 스크롤에 적용 되어짐
            ...
            var M_careerS = 1;
				...
				if(M_futureV == 1){
					M_futureV = 2;
					// ↓ future visual 동작 부분
					$(".future_visual p:eq(0) img").stop().animate({opacity:"1"}, 1000,function(){
						$(this).delay(1000).stop().animate({opacity:"0"}, function(){
							$(this).attr("src", "img/signature_ready.png").stop().animate({opacity:"1"},1500, function(){
								$(this).css({opacity:"0.5"}).attr("src", "img/signature.png").stop().animate({opacity:"1"},function(){ 
									$(".future_visual p:eq(1) img").stop().animate({opacity:"1"},1000, function(){
										// ↓ future visual 명언 동작 부분
										$(".future_visual q:eq(0)").stop().animate({marginTop:"0px",opacity:"1"},1500);							  									
										$(".future_visual q:eq(1)").delay(1000).stop().animate({opacity:"1"},2000);
										$(".future_visual cite").delay(1500).stop().animate({opacity:"1"},1000, function(){
											// ↓ future visual 프로필 동작 부분
											$(".future_visual li:eq(0)").stop().animate({opacity:"1"},1500);											 									 
											$(".future_visual li:eq(1)").delay(500).stop().animate({opacity:"1"},1500);
											$(".future_visual li:eq(2)").delay(1000).stop().animate({opacity:"1"},1500);
											$(".future_visual li:eq(3)").delay(1500).stop().animate({opacity:"1"},1500);
										});	
									});
								});
							});
				...
				//------------------------------------------------- 플랫폼 관계(O) & 너비 관계(O) & 스크롤 관계(O) 동작 실행 함수 목록
						...
						$(window).scroll(function(){
							...
							if(scNum > 700 && scNum <= 800 && bwWidth > 1100){
							  ...
							  // now main motion
							if(scNum > 3450 && bwWidth > 1100){
								mainMotion2_1();
							};
							  // future main motion
							if(scNum > 5500 && bwWidth > 1100){
								mainMotion3_1();
							};
							...
							// skill
							if(scNum > 2100 && bwWidth > 1100){ careerS_Motion() };
							// certificate
